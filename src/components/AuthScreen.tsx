...
export default function AuthScreen({ onLogin, existingUsers, onRegister }: AuthScreenProps) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      setSuccess("Autenticado com sucesso via Conta Google!");
    } catch (err: any) {
      console.error("Erro Google Sign-In:", err);
      let errorMsg = "Erro na autenticação via Google: " + (err.message || "Tente novamente.");
      if (err.code === "auth/popup-closed-by-user") {
        errorMsg = "O login via Google foi cancelado antes de ser concluído.";
      }
      setError(errorMsg);
    } finally {
      setLoading(false);
              ...
              <div className="space-y-3">
                ...
                <div className="relative my-3">
                  ...
                  <div className="relative flex justify-center text-xs uppercase">
                    <span className="bg-white px-2 text-slate-400 font-semibold">Ou acesse com</span>
                  </div>
                </div>
                ...
                <button
                  ...
                  disabled={loading}
                  className="w-full flex items-center justify-center gap-2.5 py-2.5 px-4 border border-slate-300 rounded-lg shadow-sm text-sm font-semibold text-slate-700 bg-white hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-indigo-500 transition duration-150 cursor-pointer disabled:opacity-50"
                >
